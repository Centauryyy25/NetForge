...
export interface DotPatternProps {
  ...
  /**
   * Base dot color (hex).
   * If omitted, it adapts automatically:
   *   dark  → #404040
   *   light → #a0a0a0
   */
  baseColor?: string
   ...
   * Defaults to #A981FF for both modes.
   */
  glowColor?: string
...
export function DotPattern({
  ...
  /**
   * isDark must start as `false` so server HTML and the first client render
   * agree (no hydration mismatch). After mount we sync to the `dark` class on
   * <html>, which is what ThemeToggle toggles via classList.toggle("dark").
   * A MutationObserver keeps us live whenever the class changes.
   */
  ...
  useEffect(() => {
    ...
    // Read the current class immediately after hydration
    sync()

    // Watch for future class changes triggered by ThemeToggle
  ...
  // Resolved colors: caller-supplied values take precedence
  const resolvedBase = baseColor ?? (isDark ? "#404040" : "#a0a0a0")
  const resolvedVignetteBg = isDark
    ? "radial-gradient(ellipse at center, transparent 0%, transparent 40%, rgba(10,10,10,0.6) 100%)"
    : "radial-gradient(ellipse at center, transparent 0%, transparent 40%, rgba(240,240,240,0.6) 100%)"
  ...
  return (
    <div
      ...
      className={cn(
        ...
        // Adaptive background: dark mode → near-black, light mode → near-white
    ...
    >
      ...
      {/* Vignette overlay — color adapts to current mode */}
      <div
        className="pointer-events-none absolute inset-0"
        style={{ background: resolvedVignetteBg }}
      />
